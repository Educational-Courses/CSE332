...
class QueueStack {
    create a variable of type Queue, called "stack"
    
    QueueStack() {
        construct a new Queue and assign it to variable "stack"
    }

    void push (double d) {
        enqueue d into stack
        if (size of stack is greater than 1) {
            assign 0 to a variable "i"
            create a variable "temp"
            while (i is smaller than size of stack - 1) {
                dequeue stack and store the value returned to variable "temp"
                enqueue temp into stack
                increase i by 1
            }
        }
    }
    ...
    double pop () {
        ...
        dequeue stack and store the value returned to variable "value"
        return value
    }
}
